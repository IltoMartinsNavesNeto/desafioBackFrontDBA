...
-- Group by and HAVING
SELECT 
	--EXTRACT(MONTH FROM SOL.DATA_REGISTRO) AS MES, 
	--EXTRACT(YEAR FROM SOL.DATA_REGISTRO) AS ANO,
	TO_CHAR(SOL.DATA_REGISTRO, 'YYYY-MM') AS ANO_MES,
	COUNT(*) AS REGISTROS, 
	SUM(SOL.VALOR_AUX),
	ROUND(AVG(VALOR_AUX), 2),
    COUNT(*) FILTER (WHERE SOL.STATUS = 'FINALIZADO') AS QTD_FINALIZADO
FROM 
	SOL_AUX SOL
GROUP BY
	--ANO, MES, 
...
BEGIN
    -- BUSCA STATUS ATUAL DA SOLICITAÇÃO
    SELECT STATUS
    INTO V_STATUS_ATUAL
    FROM SOL_AUX
    WHERE ID_AUX = P_ID_AUX;
    ...
    IF V_STATUS_ATUAL = 'PAGO' THEN
        P_MENSAGEM := 'ERRO: SOLICITAÇÃO JÁ FOI PAGA E NÃO PODE SER CANCELADA.';
        RETURN;
    END IF;
    ...
    VALUES (
        'SOLICITAÇÃO ID ' || P_ID_AUX || ' CANCELADA. MOTIVO: ' || P_MOTIVO,
        CURRENT_TIMESTAMP
    );

    P_MENSAGEM := 'SUCESSO: SOLICITAÇÃO CANCELADA COM ÊXITO.';
...
-----------------------------------------------------------------
-- SUBCONSULTA e WINDOW FUNCTION

WITH GASTOS_FUNCIONARIO AS (
    SELECT
        F.MATRICULA,
        F.NOME,
        F.DEPARTAMENTO,
        COUNT(S.ID_AUX) AS QTD_SOLICITACOES,
        SUM(S.VALOR_AUX) AS TOTAL_GASTO
    FROM FUNCIONARIOS F
    JOIN SOL_AUX S
        ON S.MATRICULA = F.MATRICULA
    JOIN PAGAMENTOS P
        ON P.ID_AUX = S.ID_AUX
...
SELECT
    ...
    RANK_DEPTO AS RANKING_NO_DEPARTAMENTO,
    ROUND((TOTAL_GASTO / TOTAL_DEPTO) * 100, 2) AS PERCENTUAL_DO_DEPARTAMENTO
FROM RANKING_DEPTO
WHERE RANK_DEPTO <= 3
ORDER BY
    DEPARTAMENTO,
